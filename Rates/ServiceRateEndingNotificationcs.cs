...
{
    /// <summary>
    /// Post Insert/Post Update Event on Service Rate that will create a Batch Notification Trigger
    /// for 30 days prior to end date of the Service Rate.
    /// </summary>
    public class ServiceRateEndingNotificationcs : AMCaseValidateCustomEvent
    {
        public override string PrefixName => "[NMImpact] Financials";
        public override string ExactName => "Service Rate Ends in 30 days Notification";
            ...
            RecordInstanceData preSaveRecordData, Dictionary<string, DataListData> datalistsBySystemName, Dictionary<string, Dictionary<string, FieldData>> fieldsBySystemNameByListName, string triggerType)
        {
            ...
            var serviceRateAgreementEndDate = serviceRate.Enddate;
            ...
            var nextTriggerDate = serviceRateAgreementEndDate.Value.AddDays(1);
            ...
            var notificationTriggerRecord = BatchUtils.GetNotificationTriggerRecordsOfTypeBySource(eventHelper, recordInsData,
                NotificationTriggerDatalist.FieldValues.NotificationType.ServiceRateExipired,
            ...
            {
                // Send notification
                SendServiceRateEndingNotification(eventHelper, triggeringUser, workflow, serviceRate);
            ...
            else if (today < nextTriggerDate)
            {
                ...
                else
                {
                    ...
                    BatchUtils.CreateNotificationTrigger(eventHelper, nextTriggerDate.ToString(MCaseEventConstants.DateStorageFormat), today.ToString(MCaseEventConstants.DateStorageFormat),
                        recordInsData, F_servicerateStatic.SystemName, recordInsData, F_servicerateStatic.SystemName,
                        NotificationTriggerDatalist.FieldValues.NotificationType.ServiceRateExipired);
                }
            }
            else
