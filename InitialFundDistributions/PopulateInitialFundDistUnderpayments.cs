...
{
    /// <summary>
    /// Post Update Event on Underpayments Records, triggered when Underpayment is Approved
    /// Find Fund Balance, and Create Initial Fund Distribution Record
    /// </summary>
    public class PopulateInitialFundDistUnderpayments : AMCaseValidateCustomEvent
    {
        public override string PrefixName => "[NMImpact] Financials";
        public override string ExactName => "Populate Initial Fund Distribution for Underpayments";
            ...
            RecordInstanceData preSaveRecordData, Dictionary<string, DataListData> datalistsBySystemName, Dictionary<string, Dictionary<string, FieldData>> fieldsBySystemNameByListName, string triggerType)
        {
            ...
            if (!double.TryParse(underPaymentRecord.Correctedamount, out double correctedAmount) ||
                !double.TryParse(underPaymentRecord.Originalamount, out double originalAmount))
            {
                return new EventReturnObject(EventStatusCode.Success);
            }

            underpaymentAmount = Math.Round(correctedAmount - originalAmount, 2);
            #endregion

            _initialAmountOwed = underpaymentAmount;
            _remainingServiceAmountOwed = underpaymentAmount;
            _fundsUsedCounter = 0;
            ...
            var serviceCatalogRecord = underPaymentRecord?.Correctedfund();
            ...
            {
                serviceCatalogRecord = serviceUtilization.Servicecatalogtype();
            }
            ...
            if (fundAllocationRecords == null || fundAllocationRecords.Count() == 0)
            {
                ...
                CreateBlankInitialFundDisRecord(eventHelper, serviceUtilization, errorMsg, ref _fundsUsedCounter, underPaymentRecord);
                return new EventReturnObject(EventStatusCode.Success);
            }
            ...
            foreach (var fundAllocation in fundAllocationRecords)
            {
                ...
                if (iVE == F_fundallocationStatic.DefaultValues.Eligible)
                {
                    ...
                    HandleIVEEligibleFund(eventHelper, triggeringUser, serviceUtilization, fundAllocation, today, persons, county, placement, ref _fundsUsedCounter, ref _initialAmountOwed, ref _remainingServiceAmountOwed, underPaymentRecord);
                }
                else if (iVE == F_fundallocationStatic.DefaultValues.Reimbursable)
                {
                    ...
                    HandleIVEReimbursibleFund(eventHelper, triggeringUser, serviceUtilization, fundAllocation, persons, county, iVEEligibility, maximumTitleIVEReimbursibleAmount, ref _fundsUsedCounter, ref _initialAmountOwed, ref _remainingServiceAmountOwed, underPaymentRecord);
                }
                else if (iVE == F_fundallocationStatic.DefaultValues.Reimbursable && (percentageType == F_fundallocationStatic.DefaultValues.Fmap || percentageType == F_fundallocationStatic.DefaultValues.Fmapremainder))
                {
                    ...
                    HandleIVEReimbursibleFMAPFund(eventHelper, triggeringUser, serviceUtilization, fundAllocation, persons, county, ctFMAPRecord, maximumTitleIVEReimbursibleAmount, ref _fundsUsedCounter, ref _initialAmountOwed, ref _remainingServiceAmountOwed, underPaymentRecord);
                }
                else if (iVE == F_fundallocationStatic.DefaultValues.Reimbursable && (percentageType == F_fundallocationStatic.DefaultValues.Tribalfmap || percentageType == F_fundallocationStatic.DefaultValues.Tribalfmapremainder))
                {
                    ...
                    HandleIVEReimbursibleTribalFMAPFund(eventHelper, triggeringUser, serviceUtilization, fundAllocation, persons, county, ctTribalFMAPRecord, maximumTitleIVEReimbursibleAmount, ref _fundsUsedCounter, ref _initialAmountOwed, ref _remainingServiceAmountOwed, underPaymentRecord);
                }
                else if (iVE == F_fundallocationStatic.DefaultValues.Reimbursablewithsignedvssa)
                {
                    ...
                    HandleIVEReimbursibleSignedVSSAFund(eventHelper, triggeringUser, serviceUtilization, fundAllocation, persons, county, placement, iVEEligibility, maximumTitleIVEReimbursibleAmount, ref _fundsUsedCounter, ref _initialAmountOwed, ref _remainingServiceAmountOwed, underPaymentRecord);
                }
                else
                {
                    var percentToPay = fundAllocation.Percentage;
                    if (string.IsNullOrWhiteSpace(percentToPay)) percentToPay = 100.ToString();
                    HandleFundAllocations(eventHelper, triggeringUser, fundAllocation.Fund(), persons, serviceUtilization, county, fundAllocation, percentToPay, ref _initialAmountOwed, ref _remainingServiceAmountOwed, ref _fundsUsedCounter, 0, underPaymentRecord);
                }
